...
enum class Mode{Release,Verbose,UD};
enum class Color{enable,unable,UD};
...
decltype(EXIT_FAILURE) output(jpezy::decoder<T>& dec,const char* output)
{
	...
	std::cout << "Decoded image: " << "Netpbm image data, size = " << dec.pr.template get<jpezy::property::At::HSize>() << " x " << dec.pr.template get<jpezy::property::At::VSize>() << ", pixmap, ASCII text" << std::endl;
	return EXIT_SUCCESS;
}
...
{
	if(argc > 5 or argc < 2)return disp_error();

	Mode m = Mode::UD;
	Color c = Color::UD;

	const std::string_view sv0 = argv[1],sv1 = argv[2];
	srook::optional<std::string_view> sv2 = srook::nullopt,sv3 = srook::nullopt;
	if(argc > 2)sv2 = argv[3];
	if(argc > 3)sv3 = argv[4];
	
	if((sv0.find("jpeg",sv0.find_first_of('.')) != std::string_view::npos or sv0.find("jpg",sv0.find_first_of('.')) != std::string_view::npos) and 
	(sv1.find("ppm",sv1.find_first_of('.')) != std::string_view::npos)){
		...
		if(sv2){
			if((sv2.value().find("--gray") != std::string_view::npos)){
				...
				if(sv3.value().find("--gray") != std::string_view::npos)c = Color::unable;
			}
			
			if((sv2.value().find("-v") != std::string_view::npos) or (sv3.value().find("-v") != std::string_view::npos)){
				m = Mode::Verbose;
			}else if(sv3){
				if(sv3.value().find("-v") != std::string_view::npos){
					m = Mode::Verbose;
				}
			}
		}
	...
	switch(m){
		case Mode::UD:
			return disp_error();
		case Mode::Release:[[fallthrough]];
		case Mode::Verbose:{
			switch(c){
				case Color::enable:[[fallthrough]];
				case Color::unable: 
