...
struct decoder {
	...
	comp[1].resize(unit_size, 0x80);
	comp[2].resize(unit_size, 0x80);
...
private:
	...
	if (restart_interval) {
		...
		if (mark >= MARKER::RST0 and mark <= MARKER::RST7)
	...
	do {
	    ...
	    if (tc > 1) {
		throw std::runtime_error("DC format error");
	    }
	    if (th > 3) {
		throw std::runtime_error("AC format error");
	    }
	...
	for (auto[code, length, next, k] = std::make_tuple(0, 0u, 0, 0u); k < ht_.size() and length < (block_size * 2);) {
		...
		if (ht_.codeTP[k] == code)
		    return ht_.valueTP[k];
	    }
	}
	throw std::runtime_error(__func__);
    }
